...
`phui` is already a strong human review surface, while terminal services and coding agents are better at long-running automation. The useful composition is not to make a bot keyboard-drive the TUI or to duplicate the GitHub API behind a new MCP server. It is to let an agent hand an exact pull request to a persistent `phui` pane when human judgment is needed, then continue from authoritative GitHub state after the review.
...
A local feasibility smoke on 2026-07-24 used released `phux 0.3.0` and `phui 0.10.0`: `phui` rendered correctly in a detached phux terminal, `phux snapshot --json` returned its 80x24 screen, and `phux send-keys ... a` opened the selected PR's Actions view. Terminal-level composition works today.

## What we'd ship

1. **Supported launch targets in phui**
   ...
   - Keep ordinary `phui` startup unchanged when no target is supplied.

2. **A documented phux handoff recipe**
   - Launch phui beside an exact agent pane with `phux spawn --target ... --split vertical -c <repo> -- phui owner/repo#123 --view diff`.
   - Pair the launch with `phux ask` when the agent needs human judgment.
   - Have the agent observe review decisions, comments, and checks through `gh` or the GitHub API rather than scraping phui's screen.

3. **Useful terminal identity**
   - Update the terminal title from live phui state, for example `phui · owner/repo#123 · diff`.
...
4. **Optional declarative phux integration**
   - Provide an integration template with `kind = "terminal-tui"`, `[launch].command = ["phui"]`, and `working_directory = "workspace"` so an enabled plugin can expose `phux launch phui`.
   - Keep direct `phux spawn -- phui ...` as the dependency-free contract; phui must not require phux.
...
agent works in pane @7
  ...
  -> spawns phui beside @7, targeted at owner/repo#123 --view diff
human reviews/comments/approves in phui
...
Add a tiny terminal-presentation adapter that derives a title from selected repository/item and active full-screen view. Write OSC 0 only when the derived title changes, and reset to `phui` when returning to the home surface. This remains terminal-standard behavior with no phux import or runtime dependency.
...
targets phux 0.3.0 or newer, and `integrations/phui.toml` exposes the same
`["phui"]` argv through open-vocabulary kind `terminal-tui` in the caller's
workspace:

```sh
phux plugin link ./phux-plugin/phux-plugin.toml
phux plugin enable phui
phux launch phui -- owner/repo#123 --view diff
```

This setup is manual; installing phui through Homebrew does not install or
enable the plugin. The adapter receives no GitHub credentials. The spawned
phui process continues to use the authenticated `gh` CLI exactly as it does
outside phux.

### Agent boundary

Do not expose phui's internal command registry, React handoffs, or screen text as the semantic automation API. The existing `GitHubService` separation is useful implementation structure, but the package currently exports only the `phui` executable and has no supported library or JSON service contract. For v1, bots use `gh`/GitHub API for durable actions and state; phui owns human interaction.
...
3. The optional integration ships as the small phui-owned plugin in `phux-plugin/`.
...
- A `phui-mcp` server or a second GitHub automation API.
- Bots navigating the TUI with key sequences for merge, review, or comment actions.
- A remote-control socket for retargeting an already-running phui process.
- Automatic merge or approval policy.
- phux-specific imports, protocol frames, or required configuration in phui.
...
initial views, and terminal title identity are part of phui itself with no
